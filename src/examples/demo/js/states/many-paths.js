...
class StartState extends Phaser.State {
    ...
    _addFollowers(location, num) {
        for (let i = 0; i < num; i++) {
            const follower = new FollowerSprite(this.game, location.x, location.y, this.navMesh,
                this.wallLayer);
    ...
    _isPointEmpty(x, y) {
        const checkTile = this.tilemap.getTileWorldXY(x, y, this.tilemap.tileWidth, 
            this.tilemap.tileHeight, this.wallLayer, true);
