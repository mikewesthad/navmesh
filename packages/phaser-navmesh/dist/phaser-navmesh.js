...
class phaser_navmesh_PhaserNavMesh {
  /**
  ...
  findPath(startPoint, endPoint, PointClass = external_root_Phaser_commonjs_phaser_commonjs2_phaser_amd_phaser_default.a.Geom.Point) {
    ...
    navPolys.forEach(poly => {
      ...
      if (drawNeighbors) {
        ...
        poly.neighbors.forEach(n => {
          this.debugGraphics.lineBetween(poly.centroid.x, poly.centroid.y, n.centroid.x, n.centroid.y);
        });
      }

      if (drawPortals) {
        this.debugGraphics.lineStyle(10, 0x000000);
        poly.portals.forEach(portal => this.debugGraphics.lineBetween(portal.start.x, portal.start.y, portal.end.x, portal.end.y));
      }
    });
  }
  /**
  ...
  debugDrawPath(path, color = 0x00ff00, thickness = 10, alpha = 1) {
    ...
    if (path && path.length) {
      ...
      this.debugGraphics.strokePoints(path); // Draw circle at start and end of path
...
class phaser_navmesh_plugin_PhaserNavMeshPlugin extends external_root_Phaser_commonjs_phaser_commonjs2_phaser_amd_phaser_default.a.Plugins.ScenePlugin {
  ...
  buildMeshFromTiled(key, objectLayer, meshShrinkAmount = 0) {
    ...
    const objects = objectLayer ? objectLayer.objects : []; // Loop over the objects and construct a polygon - assumes a rectangle for now!
    ...
    const mesh = new phaser_navmesh_PhaserNavMesh(this, key, polygons, meshShrinkAmount);
...
/* concated harmony reexport PhaserNavMesh */__webpack_require__.d(__webpack_exports__, "PhaserNavMesh", function() { return phaser_navmesh_PhaserNavMesh; });


/* harmony default export */ var index = __webpack_exports__["default"] = (phaser_navmesh_plugin_PhaserNavMeshPlugin);
